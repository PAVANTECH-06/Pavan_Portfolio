...
(function () {
  function ready(fn) {
    if (window.PORTFOLIO_DATA) document.addEventListener("portfolio:ready", fn);
  }
  ...
  ready(function () {
    ...
    const rules = [
      ...
      {
        test: /educat|degree|college|cgpa|university|school/i,
        answer: () =>
          data.education.map((e) => `${e.title} — ${e.org} (${e.period})`).join("\n"),
      },
      {
